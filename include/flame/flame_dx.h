...
// =============================================================================
//  USAGE EXAMPLE — included as a comment block; strip before shipping.
// =============================================================================
//
// ── Example 1: Basic conversion round-trip ──────────────────────────────────
//
//   #include "flame_dx.h"
//   using namespace flame;
//   using namespace flame::dx;
//
//   Vec3 position = { 10.f, 5.f, -3.f };
//
//   // flame  → DirectX
//   XMVECTOR dxPos  = ToDX(position);                  // 1 aligned load
//   XMVECTOR dxPosW = ToDXPoint(position);             // 1 load + 1 blend
//
//   // DirectX → flame
//   Vec3 back = FromDX(dxPos, AS_VEC3);                // 1 store
//
//
// ── Example 2: Camera matrix pipeline ───────────────────────────────────────
//
//   Vec3 eye    = { 0, 5, -10 };
//   Vec3 target = { 0, 0,   0 };
//   Vec3 up     = { 0, 1,   0 };
//
//   // Build in flame (cheap; stays in SIMD regs)
//   Mat4 view = LookAt(eye, target, up);
//   Mat4 proj = Perspective(60.f, 16.f/9.f, 0.1f, 1000.f);
//   Mat4 vp   = MUL(proj, view);          // flame multiplication
//
//   // Hand off to DX pipeline — zero-copy reinterpret
//   XMMATRIX dxVP = ToDX(vp);
//   // OR use the helper that avoids materialising an intermediate flame Mat4:
//   XMMATRIX dxVP2 = BuildViewProjectionDX(eye, target, up, 60.f, 16.f/9.f, 0.1f, 1000.f);
//
//
// ── Example 3: D3D12 constant buffer (zero-conversion upload) ───────────────
//
//   struct alignas(256) PerObjectCB {   // D3D12 requires 256-byte CB alignment
//       Mat4 worldViewProj;             // 64 bytes — binary-identical to XMMATRIX
//       Mat4 world;
//       Vec4 color;
//   };
//
//   PerObjectCB cbData;
//   cbData.worldViewProj = MUL(vp, entity.transform.ToMat4());
//   cbData.world         = entity.transform.ToMat4();
//   cbData.color         = { 1, 0.5f, 0, 1 };
//
//   // Upload — no XMStoreFloat4x4, no scalar loop, just memcpy
//   memcpy(pUploadHeap, &cbData, sizeof(cbData));
//
//
// ── Example 4: Frustum culling a DX view-projection matrix ──────────────────
//
//   XMMATRIX dxViewProj = ...; // From your existing DX pipeline
//   Frustum  f          = FrustumFromDX(dxViewProj);
//
//   for (auto& entity : entities) {
//       if (!FrustumCullAABB(f, entity.worldBounds))
//           RenderEntity(entity);
//   }
//
//
// ── Example 5: Batch position transform ─────────────────────────────────────
//
//   // 1024 world positions already in a DX vertex stream (XMVECTOR[])
//   alignas(16) XMVECTOR worldPositions[1024];
//   alignas(16) XMVECTOR screenPositions[1024];
//
//   Mat4 mvp = MUL(proj, MUL(view, model));
//   TransformPointsBatchDX(mvp, worldPositions, screenPositions, 1024);
//   // No XMMATRIX materialisation; Mat4 drives the loop directly.
//
//
// ── Example 6: Interop with DX quaternion operations ────────────────────────
//
//   Quat flameRot = QuatFromAxisAngle({ 0,1,0 }, ToRadians(45.f));
//
//   // Hand to DX for slerp (or any other DX quat operation)
//   XMVECTOR dxQ    = ToDX(flameRot);
//   XMVECTOR dxId   = XMQuaternionIdentity();
//   XMVECTOR dxSlerp = XMQuaternionSlerp(dxId, dxQ, 0.5f);
//
//   // Bring result back into flame world
//   Quat result = FromDX(dxSlerp, AS_QUAT);
//   Vec3 rotated = QuatRotate(result, { 1, 0, 0 });
//
//
// ── Performance notes ────────────────────────────────────────────────────────
//
//   TYPE        ToDX COST         FromDX COST      NOTES
//   Vec2        1 load            1 store+2 write   w/z zeroed
//   Vec3        1 load            1 store+1 write   w zeroed
//   Vec4        1 load            1 store           identical layout
//   Quat        1 load            1 store           identical layout
//   Mat4        0 (reinterpret)   4 reg copies      binary identical
//   Transform   TRS + reinterpret  —                bakes on demand
//
//   Chained DX↔flame↔DX conversions for Vec4/Quat/Mat4 have ZERO cost because
//   the compiler folds the load↔store into a register rename at -O2 or higher.
//   Verify with Compiler Explorer (godbolt.org) targeting clang -O2 -mavx2.
//
// ── Alignment checklist before shipping ─────────────────────────────────────
//
//   ✓ All flame heap allocations: use aligned_alloc(16, size) or _aligned_malloc
//   ✓ Stack-allocated structs: compiler handles alignas(16) automatically
//   ✓ STL containers: use a custom allocator (e.g. flame::AlignedAllocator<T,16>)
//   ✓ GPU upload heaps: D3D12 CB alignment 256 bytes, use alignas(256) on CB struct
//   ✓ NEVER pass flame types by value through virtual interfaces (defeats alignment)
//
// =============================================================================